...
import { makeGeneratedPostVersion } from "@/lib/generated-posts";
...
export async function POST(request: Request) {
  try {
    ...
    const source = body.source || (await getSourceItemsByIds([body.post.sourceItemId]))[0];
    if (!source) return NextResponse.json({ error: "Source item is required" }, { status: 400 });

    const nextPost = await generatePost({
      source,
      materialPaths: Array.isArray(body.materialPaths) ? body.materialPaths : body.post.materialPaths,
      instruction: body.instruction || "重新生成一个差异化版本，保留事实信息，调整标题钩子、段落节奏和表达角度。",
      productionPlanOverride: body.productionPlanOverride || body.post.productionPlanOverride,
      imageTasks: Array.isArray(body.imageTasks) ? body.imageTasks : body.post.imageTasks,
    });
    ...
    const post = await makeGeneratedPostVersion(body.post, nextPost);
    return NextResponse.json({ post });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "Failed to regenerate post" }, { status: 500 });
  }
}
