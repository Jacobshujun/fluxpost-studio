...
import type {
  LibraryAsset, LibraryAssetFilters, LibraryAssetPage, LibraryBatchResult, LibraryCollection, LibraryCollectionBatchRequest, LibraryCollectionBatchResult, LibraryListSort, LibraryNavigation,
...
export default function LibraryPage() {
  ...
  async function deleteAsset(asset: LibraryAsset) {
    if (busy || !asset.canEdit) return;
    setBusy(true);
    setDeleteError("");
    setMessage("");
    try {
      const result = await api<LibraryBatchResult>("/api/library/assets/batch", {
  ...
  return <main className={styles.page} onDragEnter={(event) => { if (hasFiles(event)) setDragging(true); }} onDragOver={(event) => { if (hasFiles(event)) event.preventDefault(); }} onDrop={(event) => { event.preventDefault(); setDragging(false); void importFiles([...event.dataTransfer.files]); }}>
    <header className={styles.header}><div className={styles.brand}><Link href="/" className={styles.iconButton} title="返回内容台"><ArrowLeft /></Link><div><strong>图库</strong><span>{title} · {data.total ?? 0} 张</span></div></div><div className={styles.headerActions}><button className={styles.iconButton} title="刷新" onClick={() => void Promise.all([reloadAssets(), loadNavigation()])}><RefreshCw /></button><button className={styles.primary} onClick={() => fileInputRef.current?.click()}><Upload />导入</button><input ref={fileInputRef} hidden multiple accept="image/*" type="file" onChange={(event) => { void importFiles([...event.target.files || []]); event.target.value = ""; }} /></div></header>
...
function DetailPanel({ asset, collections, busy, onClose, onSave, onFavorite, onPreview, onDelete, deleteError }: { asset: LibraryAsset; collections: LibraryCollection[]; busy: boolean; onClose: () => void; onSave: (patch: Record<string, unknown>) => void; onFavorite: () => void; onPreview: () => void; onDelete: () => void; deleteError: string }) {
