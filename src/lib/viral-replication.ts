...
export async function recordViralImageTaskPlan(input: {
  ...
  await recordExecutionLog({
    ...
    status: "info",
    message: `Prepared ${selectedTasks.length} viral image task(s) with ordered vehicle and source references.`,
    details: {
      ...(input.runId ? { runId: input.runId } : {}),
      taskCount: selectedTasks.length,
      pairedImageCount: input.pairs.length,
      referenceShape: selectedTasks.length ? "ordered_references" : "none",
