...
export async function planCanvasRunWithMode(
  ...
  for (const nodeId of plan.confirmationNodeIds) {
    ...
    if (node.type !== "model.seedance") {
      details.push({ nodeId, label: node.label?.trim() || definition?.label || node.type, status: "ready" });
      continue;
    }
    try {
...
async function runPlannedNode(
  ...
  if (missingInput) {
    ...
    const nodeRun = await saveTerminalNodeRun(
      ...
      "blocked",
      {},
      `${node.label?.trim() || definition?.label || node.id}: Missing required input ${missingInput}.`,
      inputs,
    );
    return { nodeId: node.id, nodeRun };
  }
...
async function runReusedNode(
  ...
  const nodeRun = await saveCanvasNodeRunToDb({
    ...
    internalMetadata: source.internalMetadata ? structuredClone(source.internalMetadata) : undefined,
