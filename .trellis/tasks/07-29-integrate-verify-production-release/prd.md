...
- The working tree also contains 47 modified tracked files plus untracked source, task, verification, and local debug artifacts. These changes span several existing Trellis tasks and cannot be treated as one automatically approved release scope.
...
- R12. Limit the candidate feature scope to the latest GitHub baseline plus the completed Canvas/copy-batch workflow, member library upload, and shared image/copy library sorting and batch-selection work, together with only the integration fixes required for those features.
- R13. Exclude unfinished or unverified tasks, legacy local-material removal, local debug artifacts, and infrastructure/spec changes that are not required by the approved feature scope.
...
- [ ] AC10: The release inventory contains only the approved completed feature set and its required integration fixes; excluded unfinished work remains outside the candidate. (R12-R13)
...
- Unfinished or unverified feature tasks, including legacy local-material removal.
