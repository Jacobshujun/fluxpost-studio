...
- [ ] Refresh remote refs and assert `origin/main` still matches the planned base and is an ancestor of `d05cadd`.
- [ ] Create an isolated release worktree and dedicated branch from `d05cadd`; leave root `main` and all root working-tree files unchanged.
- [ ] Port the member upload queue-id source/check hunks without stale status metadata.
- [ ] Port only the shared-library task's owned source, styles, helper modules, and focused checks; preserve legacy-material migration, remote library contracts, and unrelated image/tag behavior.
- [ ] Port the verified legacy local-material retirement as a dedicated commit, adapting overlapping library/types/database hunks to the candidate instead of copying whole dirty files.
- [ ] Port local Next build slimming as a dedicated commit and retain the candidate's newer deployment scripts.
- [ ] Port Canvas condition-random unique copy assignment as a dedicated commit.
- [ ] Port bounded Canvas queue concurrency and Canvas run/retry reliability as reviewable hunks, preserving provider-pool limits and existing durable queue contracts.
- [ ] Exclude root VPS wrapper v2 changes, Feishu table-id/CLI changes, review-upload delegation, broad verification rewrites, screenshots, runtime data, and unrelated specs.
- [ ] Add this release task and only stable spec facts needed by the candidate.
- [ ] Review `git diff --check`, changed paths, ignored files, staged files, and object sizes; map every candidate path to an approved requirement.

## Candidate Verification

- [ ] Run focused library-assets, copy-library, legacy-material retirement, local-build-output, Canvas scheduler/workflow/concurrency, and VPS deployment checks.
- [ ] Run `npx --no-install tsc --noEmit`, `npm run lint`, and `npm run build`.
- [ ] Run mocked desktop/mobile library browser verification without external services.
