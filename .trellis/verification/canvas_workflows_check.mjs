...
try {
  writeFileSync(path.join(temp, "toapis-image-api.js"), `exports.toApisImageResolutions=['1k','2k','4k']; exports.toApisImageRatios = ${JSON.stringify(["1:1", "3:2", "2:3", "4:3", "3:4", "5:4", "4:5", "16:9", "9:16", "2:1", "1:2", "21:9", "9:21"])}; exports.toApis4kImageRatios = ${JSON.stringify(["16:9", "9:16", "2:1", "1:2", "21:9", "9:21"])};`, "utf8");
  ...
  for (const name of ["types", "node-utils", "source-video-contract", "video-loader", "content-collection", "save-images", "seedance-references", "subtitle-style", "subtitle-editor", "registry", "graph", "iteration", "serialization", "clipboard", "workflow-file"]) {
...
const withCompetitorWorkbookReadiness = compileFunctions(
  ...
  "withCompetitorWorkbookReadiness",
  { getCanvasNodeExecutionMode: (node) => node.executionMode || "enabled" },
);
const workbookPlan = { includedNodeIds: ["workbook"], blockers: [], preflightBlocked: false };
const missingWorkbookSnapshotPlan = withCompetitorWorkbookReadiness({
  nodes: [{ id: "workbook", type: "input.competitor-workbook", version: 1, config: {} }],
  edges: [],
  viewport: { x: 0, y: 0, zoom: 1 },
}, workbookPlan);
assert.equal(missingWorkbookSnapshotPlan.preflightBlocked, true);
assert.equal(missingWorkbookSnapshotPlan.blockers[0]?.code, "competitor_workbook_snapshot_required");
assert.equal(withCompetitorWorkbookReadiness({
  nodes: [{ id: "workbook", type: "input.competitor-workbook", version: 1, config: { rowTitle: "标题", rowBody: "正文", cardText: "参数卡" } }],
  edges: [],
  viewport: { x: 0, y: 0, zoom: 1 },
}, workbookPlan).preflightBlocked, false, "a frozen ordinary-run row/card must pass readiness");
...
requireText(page, ["ReactFlow", "onConnect", "wouldCreateCycle", "NodeInspector", "panOnDrag={isMobile}", "selectionOnDrag={!isMobile}", "nodesDraggable={!isMobile}", "RunSummary", "FlowingCanvasEdge", "canvas-port-row", "colorMode={flowColorMode}", "subscribeTheme", "CANVAS_CLIPBOARD_MIME", "dataTransferImageFiles", "isEditableClipboardTarget", "pasteFromSystemClipboard", "canvas-image-file-input", "CanvasNodeInteractionContext", "latestNodeRuns", "latestNodeAttempts", "latestSuccessfulNodeRuns", "displayingExplicitRun", "runRequestPendingRef", "正在保存并检查运行条件", 'blocker.code === "competitor_workbook_snapshot_required"', "检查并冻结测试行", "打开批量调度", "请检查 Xray", "useMemo(() => displayingExplicitRun", "(result.get(nodeRun.nodeId)?.attempt || 0) < nodeRun.attempt", "const activeRunId = selectedRunIdRef.current", "const routeRun = !activeRunId && runId", "const selectedRun = explicitRun", "await refreshRun(selectedRun.id, workflowId)", "runSelectionIsExplicitRef", "focusCanvasNode", "selectedNodeId", "if (selectedNode) setSelectedNodeId(selectedNode.id)", "interaction?.selectedNodeId === node.id", "canvas-node-text-editor nodrag nopan nowheel", "event.currentTarget.focus({ preventScroll: true })", "interaction?.onNodeFocus(node.id)", "onClick={(event) => {", "onKeyDown={(event) => event.stopPropagation()}", "CanvasModelNodeResult", "CanvasImagePreviewNodeResult", "CanvasNodeAttemptDetail", "updateNodeExecutionMode", "仅运行此节点", "运行到此节点", 'requestRun([selectedNodeId], "isolated")', "打开评审", "历史版本 r", "最近成功结果 · r", "definition?.outputs", "isPreviewableModelArtifact", "artifact.value.trim()", "artifact.items.length > 0", "showArtifact", "运行完成，但没有可预览内容", "CanvasTextPreviewDialog", "CanvasVideoPreviewDialog", "CanvasImagePreviewDialog", "canvas-node-result-gallery", "canvas-node-result-gallery-open", "canvas-node-result-gallery-meta", "canvas-image-preview-open", "图片{index + 1}", "imageUrls.length}/16", "moveListItem", 'form.append("mode", "gpt-reference")', "edgeAnimationDelay", "pathLength={100}", "canvas-flow-edge-trail", "canvas-flow-edge-body", "canvas-flow-edge-core", "打开原图", "缩小图片", "放大图片", "重置图片缩放"], "canvas UI");
requireText(page, [
  "prepareCanvasClipboardPaste",
  "canvasClipboardRef",
