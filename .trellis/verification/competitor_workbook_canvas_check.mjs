...
assert.match(scheduler, /taskConcurrency: normalizeInteger[\s\S]*1, 5/);
assert.match(scheduler, /prepareCanvasScheduleV2Admissions\(next, next\.taskConcurrency \|\| 2/);
assert.match(scheduler, /retryPending: true/);
assert.match(scheduler, /child\.workbookCard\?\.cardIndex \|\| index \+ 1/);
...
assert.ok(page.includes("请人工核对图片中的中文、数字和参数"));
assert.ok(page.includes("taskConcurrency"));
