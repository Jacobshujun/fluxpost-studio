...
- Content-pool custom tags are a separate manual layer from fixed AI `contentTagging.tags`: normalize with NFKC/trim/whitespace collapse/case-insensitive keys, cap each item at 20 labels and 40 characters per label, and treat missing legacy `customTags` as `[]`.
- The authenticated `GET /api/content-pool/tags` and `POST /api/content-pool/tags` routes delegate to owner-scoped domain helpers. Batch add/remove changes only manual tags, processes each selected id independently, writes the content pool once when at least one item succeeds, and returns `{ items, failures }` with per-item errors.
- Content-pool and Canvas selection filters must share `normalizeContentPoolCustomTags`/`matchesAllContentPoolCustomTags`; `contentTag` and `customTag` dimensions both use AND semantics, and ordinary search normalizes NFKC text and includes custom labels.
...
- Ordinary 2.5 flare/sunburst JSON: `model`, `prompt`, `n:1`, ratio `size`, `resolution`, fixed `quality:high`, URL `reference_images`, optional `background:transparent`. No output-format/compression/response-format fields; returned bytes are preserved. Format/compression selections have no provider effect. Invalid counts fail before upload without retry/failover. Source: https://docs.toapis.com/docs/cn/api-reference/images/gpt-image-2.5/generation (2026-09-14). VIP/Official are separate.
- Older ToAPIs models retain n=1..10, output fields, image_urls and six-ratio 4K restrictions only within their adapter.
...
- `WorkspacePromptSettings`/`SimpleRun` persist imageRatio/imageResolution/imageSize. Canvas/simple allow every listed ratio at 4K without substitution. `pixelSizeForRatio` projects tiers to aligned pixels: square 4K=2880x2880, 16:9=3840x2160; edges divisible by 16, max edge 3840, max area 8,294,400. No local resize. ToAPIs receives ratio/tier. Legacy pixels and frozen resume values stay unchanged.
...
- Browser workflow saves use `createWorkflowSaveCoordinator`: one PATCH may be in flight, later automatic/manual intents merge by workflow and dirty version, and a queued newer snapshot adopts the prior response revision before sending. A covered manual intent upgrades acknowledgement without duplicating the PATCH. Failure keeps dirty state, drops that workflow's pending automatic work, blocks further automatic retries, and permits a later explicit manual retry. Responses may update only their matching workflow; they cannot replace another active workflow or clear edits newer than the saved snapshot.
- Workflow create/update/import first decode the untrusted graph, then use persistence validation: structurally safe incomplete drafts may be saved, while unknown nodes, malformed config shapes, bad ports, cycles, invalid dimensions, and duplicate scheduler roles remain rejected. Run planning separately uses full execution validation for required config, required inputs, and provider-specific readiness.
...
- Canvas model-style result projection checks every declared `text`, `images`, and `videos` output in port order and renders the first non-empty artifact actually produced by the selected attempt. Multi-output nodes such as `utility.media-mask` must not infer the run's artifact kind from only the first declared port; undeclared artifact kinds remain excluded. `.trellis/verification/canvas_media_mask_check.mjs` covers video-only, image-only, empty, and undeclared-kind results.
...
- `canvas_workflows_check.mjs` must cover persistence-versus-execution validation, malformed structural rejection, and deferred saves with maximum concurrency one, revision chaining, latest-snapshot follow-up, covered manual acknowledgement, automatic failure blocking, and explicit manual retry. `canvas_workflow_draft_persistence_check.mjs` must round-trip an incomplete draft through an isolated real SQLite store. `.trellis/verification/canvas_save_race_browser_check.py` must additionally assert the live save-button state and serialized delayed PATCH behavior against a mocked workflow API.
...
## Scenario: Canvas Competitor Workbook Scheduling

### 1. Scope / Trigger

- Trigger: changing `input.competitor-workbook@1`, local workbook inspection, workbook-backed Canvas V2 parameters, hierarchical row/card scheduling, or workbook response redaction.
- This path composes existing prompt, `model.gpt-image@2`, image-library, and `compose.social-post` nodes. It must not add a provider-specific image node, a second queue, or an automatic Feishu write.
...
| --- | --- |
| Missing sign-in / signed-in non-admin | HTTP `401` / `403` for inspection; workbook configuration or launch is rejected |
...
```typescript
// Wrong: keep a mutable path in a public schedule response and reread it per child.
return { schedule, filePath: source.filePath };

// Correct: freeze once at preflight and redact every API graph/schedule boundary.
const snapshot = await freezeCompetitorWorkbook(source);
return canvasScheduleResponse({ ...schedule, definition: bindSnapshot(snapshot) });
```
