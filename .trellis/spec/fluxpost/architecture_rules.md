...
- `canvas_workflows_check.mjs` plus the scoped Canvas performance browser check must assert visible-element culling, idle one-path edges, selected/running beam eligibility, pan/zoom suspension without persisted state, and reduced-motion suppression.
