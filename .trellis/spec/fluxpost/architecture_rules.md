...
- `canvas_workflows_check.mjs` must defer workflow saves and assert maximum concurrency one, revision chaining, latest-snapshot follow-up, covered manual acknowledgement, automatic failure blocking, and explicit manual retry. `.trellis/verification/canvas_save_race_browser_check.py` must additionally assert the live save-button state and serialized delayed PATCH behavior against a mocked workflow API.
