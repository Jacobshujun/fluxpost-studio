...
- 2026-08-25: Excel/generic V2 `at-least-one`/`all` parity, absence of review-layer `canvasImageBatch`, failed-only retries, review/Feishu legacy-key tolerance, SQLite dry-run/apply preservation, PostgreSQL transactional JSONB removal, TypeScript, lint, build, isolated HTTP smoke, and the full baseline passed without live providers or Feishu.
