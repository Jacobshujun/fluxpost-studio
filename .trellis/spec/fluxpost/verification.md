...
Run the cross-platform deterministic baseline from the isolated candidate worktree. The command must pass on the final committed SHA; any later content edit invalidates the evidence:
...
- Static/domain checks for PostgreSQL schema, workspace accounts, managed libraries, retired local-material surfaces, local/Docker build output, advanced config, TOS, Ubuntu VPS deployment contracts, platform/media/video behavior, concurrency, Feishu queues, simple-run policies, Canvas workflows/scheduler, copy library, review flows, source safety/importers, provider adapters, and row-level runtime mutations.
- `npm run lint`.
...
After the baseline passes, push the dedicated candidate branch and verify its remote full SHA equals the local SHA. Then run read-only preflight on production 38 and present the SHA, scope, residual risks, current release, and rollback release for explicit approval. Deployment uses only `/opt/fluxpost-studio/bin/deploy.sh --ref <full-sha>`.

## HTTP Smoke

The baseline starts an isolated local production server and runs:

```bash
node .trellis/verification/http_smoke.js http://127.0.0.1:3310
```
...
- No authenticated browser UI walkthrough is part of the default baseline; task-specific mocked or operator-approved browser acceptance remains separate.
...
- Add new baseline checks only when they are deterministic, isolated, and do not mutate production runtime data.
...
- A candidate SHA change invalidates prior baseline, remote-equality, preflight, and approval evidence. Restart those gates for the new SHA.
