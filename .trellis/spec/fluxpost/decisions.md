...
- Dongchedi (`懂车帝`) is supported only as a source-link/ID import platform, not as a TikHub keyword crawl platform. Link-import mode accepts full `https://www.dongchedi.com/ugc/article/{id}` URLs by auto-detection; pure numeric article ids such as `7643008384274546713` require the user to select `懂车帝`. The local importer fetches public article HTML, parses embedded article JSON when available, normalizes it as `platform="dongchedi"`, and fails clearly on ByteDance/Dongchedi anti-bot challenge HTML instead of importing empty items.
