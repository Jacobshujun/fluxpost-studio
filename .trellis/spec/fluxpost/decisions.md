...
- Local port `3001` is a production mirror, not a feature preview. `npm run local:restart` resolves the deployed full SHA, creates or reuses a clean SHA-specific sibling release worktree, builds before replacing the listener, injects local-production identity, and verifies `/api/version`; `npm run local:parity` then proves local/remote SHA equality, mirror cleanliness/HEAD, and `origin/main` ancestry.
- Local development stays on port `3000` and disables background workers by default; `FLUXPOST_DEVELOPMENT_WORKERS=1` is an explicit shared-state worker opt-in. Code identity is synchronized, while secrets, accounts, databases, queues, media, and provider state are not.
