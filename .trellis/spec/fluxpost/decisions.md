...
- Content tag `提车记录` remains a content-pool/tagging category and may normalize aliases such as `提车`, `提车作业`, `车主提车`, `新车交付`, and `交付记录`. Content tags support retrieval and review only; they must not classify, block, or rewrite production behavior.
...
- Remote image HTTP uses `src/lib/image-transport.ts` with an explicit Windows-local Xray default at `127.0.0.1:10808`; it does not use ambient system/env proxy discovery, does not proxy loopback/text/ComfyUI/Feishu traffic, and does not disable TLS verification.
...
- Scenario: content-pool secondary creation via `/content`.
  ...
  3. Contracts: pool runs read selected samples through `getSourceItemsByIds(sourceItemIds, simpleRunAccessActor(input))`, then reuse simple-run safety filtering, tagging, production, image, draft persistence, run progress, and review-desk flow. Text and image behavior comes only from the `WorkspacePromptSettings` frozen on the run plus explicit media switches; stored legacy `productionPlan` values and source content tags do not control production.
  ...
  5. Good/Base/Bad Cases: good = selected owner-visible samples use the frozen workspace text/image prompts and produce drafts; base = missing or inaccessible ids are counted; bad = tags or historical production plans block/reframe a source, or pool mode crawls/imports/publishes.
  6. Tests Required: `content_desk_check.mjs`, `simple_crawl_media_policy_check.mjs`, and `user_text_instruction_priority_check.mjs` assert pool payload/ownership, prompt-only production control, media eligibility, absence of automatic plan enforcement, and `writeFeishu: false` at UI and domain boundaries.
  7. Wrong vs Correct: wrong = deriving industry/competitor/Xpeng/video/pickup production rules from source content or auto-writing pool output to Feishu; correct = `/content` supplies selected samples, the frozen main-workspace prompts and switches control generation, and output enters `/review` as drafts.
...
- Port `3001` has one local application runtime: the clean committed candidate with normal background workers. `npm run local` binds it to `127.0.0.1`; `npm run local:lan` binds the same candidate contract and SHA to `0.0.0.0` without creating another version or port. The account and permission model is identical on both bindings.
- Candidate preparation installs locked dependencies while the worktree server is stopped. `npm run local` requires the primary worktree to be clean, builds the committed HEAD into the inactive ignored `.next-local-a`/`.next-local-b` slot while the prior candidate remains available, then replaces the listener, injects `candidate` plus the full HEAD, and verifies `/api/version` and HTTP health. A failed build leaves the listener untouched; failed activation restores the previously recorded slot. `local:restart` remains a compatibility alias. Only that tested SHA may be pushed and deployed. `npm run local:parity` requires exact equality among clean local HEAD/runtime, GitHub `origin/main`, and production. Secrets, accounts, databases, queues, media, configuration, and provider state are never synchronized as code.
- Default local Next builds use standard `.next` output because local production runs through `next start`. The candidate launcher alone selects one of the two allow-listed local build slots. Docker builder sets `FLUXPOST_STANDALONE_BUILD=1` to retain standalone output, and output tracing excludes runtime `data`, `public/media`, `public/generated`, and `test-artifacts` paths because production supplies runtime state through named volumes.
