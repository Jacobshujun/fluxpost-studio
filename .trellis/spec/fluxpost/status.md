...
- 2026-09-02 Canvas batch target-status fix: V1 image tasks, V2 child tasks, V2 shared stages, and V2 main targets now project status from required outputs instead of copying a mixed CanvasRun status. Missing target artifacts are lazily corrected to failed with durable errors; valid partial outputs remain partial. New regression fixtures cover upstream-only failures, empty completed outputs, shared-output gaps, and V1/V2 lazy correction. Canvas scheduler check, TypeScript, lint (0 errors, 20 warnings), build, isolated HTTP/SQLite smoke, and the full offline baseline passed.
