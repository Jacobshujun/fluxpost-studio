...
- Batch source-link import uses TikHub detail/share endpoints per platform. Default Harness coverage validates endpoint construction and integration wiring only; live URL imports still depend on TikHub response shape, platform short-link resolution, and the configured OpenAI-compatible safety/tagging path.
