...
assertContains(contentImport, /cacheCrawledMedia\(importedItems\)/, "Feishu imported items must pass through the existing media cache path.");
...
assertContains(route, /sourceMode:\s*body\.sourceMode === "feishu" \? "feishu" : body\.sourceMode === "links" \? "links" : "keyword"/, "Simple run API must forward Feishu source mode.");
...
assertContains(page, /导入飞书并一键生产内容/, "Simple start button must describe the Feishu one-click flow.");
...
assertContains(checkPs1, /Feishu content import check/, "Harness baseline must include the Feishu content import check.");
